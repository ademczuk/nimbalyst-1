...
- Voice Mode now properly requests macOS microphone permission on first connect instead of failing silently. The `com.apple.security.device.audio-input` entitlement was previously omitted to avoid an at-launch prompt, but that left `askForMediaAccess('microphone')` returning false and Nimbalyst never appearing in System Settings, Privacy and Security, Microphone. The entitlement is now present in `entitlements.mac.plist` and `VoiceModeService.test-connection` calls `askForMediaAccess('microphone')` when status is `not-determined`, triggering the standard macOS prompt. The pre-existing session permission handler in `main/index.ts` continues to deny media (audio) requests until the user has explicitly turned on Voice Mode, so no early prompt fires on app launch. `NSMicrophoneUsageDescription` was already in `package.json` `extendInfo`. Fixes #22, #214.
