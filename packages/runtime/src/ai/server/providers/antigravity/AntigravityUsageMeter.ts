 ...
 *
 * See nimbalyst-local/agy-standalone-approachB.md for captured real values.
 */
...
export class AntigravityUsageMeter {
  constructor(private readonly server: AntigravityServerManager = AntigravityServerManager.shared()) {}
  ...
  /**
   * Per-model quota. `modelKeyOrEnum` may be a stable key (resolved to the live
   * enum) or an enum. Returns null if the model is not in the user's config.
   */
