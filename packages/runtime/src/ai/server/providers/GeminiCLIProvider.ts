...
export class GeminiCLIProvider extends BaseAgentProvider {
  ...
  async *sendMessage(
    ...
    yield* this.sendMessageInternal(message, documentContext, sessionId, messages, workspacePath, attachments, false);
  }

  // Internal generator carrying the forceNoMcp self-retry flag (set after Gemini
  // rejects a turn for exceeding its 512 function-declaration limit). Kept private
  // so the public sendMessage matches the BaseAgentProvider contract.
          ...
          switch (item.kind) {
            ...
            case 'error':
              if (!forceNoMcp && !fullText && /At most 512 function declarations/i.test(item.message)) {
                ...
                yield* this.sendMessageInternal(message, documentContext, sessionId, messages, workspacePath, attachments, true);
                return;
              }
